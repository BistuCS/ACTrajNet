...
class ACTrajNet(nn.Module):
    def __init__(self, args):
        ...
        tcn_alti_out = 1   
        num_channels_alt= [args.tcn_channel_size]*args.tcn_layers
        num_channels_alt.append(tcn_alti_out)
        ...
        self.lstm_encoder_x = LstmNet(input_size=lstm_input_size, output_size=n_classes_lstm, hidden_size=lstm_hidden_size, num_layers=lstm_layers, dropout=dropout)
        self.lstm_encoder_y = LstmNet(input_size=lstm_input_size, output_size=n_classes_lstm, hidden_size=lstm_hidden_size, num_layers=lstm_layers, dropout=dropout)
    ...
    def init_weights(self):
        ...
        self.proj_X.weight.data.normal_(0, 0.05)
        self.proj_Y.weight.data.normal_(0, 0.05)
    ...
    def forward(self, x, y, adj,context,sort=False):        
        ...
        for agent in range(x.shape[2]):
            ...
            #encoded_x2.shape: torch.Size([1, 12, 11])
            encoded_x2 = self.tcn_encoder_x(x1)
            
            #altitude_x1.shape: [1, 11, 1]
            altitude_x1 = x[:, 2, agent][None, :, None] 
    
            #  encoded_alt_x.shape: torch.Size([1, 12, 11])
            encoded_alt_x = self.lstm_encoder_x(altitude_x1)
            encoded_alt_x = torch.mean(encoded_alt_x,  dim=1, keepdim=True)  # [1, 12, 11]→[1,1,11]
            ...
            encoded_x = torch.flatten(encoded_x2)[None,None,:] # shape torch.Size([1, 1, 132])
            ...
            encoded_context = self.context_linear(encoded_context)
            
            #appended_x.shape ([1, 1, 139])
            ...
            # encoded_alt_y.shape = torch.Size([1, 12, 12])
            encoded_alt_y = self.lstm_encoder_y(altitude_y1)  

            encoded_alt_y = torch.mean(encoded_alt_y,  dim=1, keepdim=True)  # [1, 12, 12]→[1,1,12]

            #encoded_alt_y :(1, 12, 12)
            encoded_alt_y_repeated = encoded_alt_y.repeat(1, 12, 1) 
    ...
    def inference(self,x,z,adj,context):
        ...
        for agent in range(x.shape[2]):
            ...
            encoded_alt_x = self.lstm_encoder_x(altitude_x)
            encoded_alt_x = torch.mean(encoded_alt_x,  dim=1, keepdim=True)  # [1, 12, 11]→[1,1,11]
