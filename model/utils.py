...
class TrajectoryDataset(Dataset):
    ...
    def __init__(
        ...
        seq_list_rel = []
        context_list = []
        for path in tqdm(all_files, ncols=80):
            data = read_file(path, delim)
            # [timestamp, id, x, y, z, wind_x, wind_y]
            # print(path)
            # print(data[:])
            # exit()
            if (len(data[:,0])==0):
            ...
            for idx in range(0, num_sequences * self.skip + 1, skip):
                # print('!1', frame_data[idx:idx + self.seq_len])
                curr_seq_data = np.concatenate(
                    frame_data[idx:idx + self.seq_len], axis=0) 
                # print('!2', curr_seq_data)
                # exit()
...
def mde(y1, y2):
    loss = (y1 - y2)**2
    loss = np.sqrt(np.sum(loss, axis=1))
    return np.max(loss)
