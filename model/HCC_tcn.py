...
class ACTrajNet(nn.Module):
    ...
    def forward(self, x, y, adj,context,sort=False):        
        ...
        for agent in range(x.shape[2]):
            # shape x : torch.Size([11, 3, 1])
            # shape x[:,:, agent][None, :, :] : torch.Size([1, 11, 3]) 
            #x1.shape(1,3,11)
            ...
            altitude_x1 = x[:, 2, agent][None, None, :] # altitude_x1.shape: [1, 1, 11]

       
            encoded_alt_x = self.tcn_encoder_altitude_X(altitude_x1) # en_alt shape: torch.Size([1, 1, 11])
            ...
            #y.shape [12, 3, 1]  y1.shape(1,3,12)
            y1 = torch.transpose(y[:,:, agent][None, :, :], 1, 2) # y shape: torch.Size([12, 3, 1])
            #altitude_y1.shape: [1, 1, 12]
            altitude_y1 = y[:, 2, agent][None, None, :] 

             #encoded_alt_y.shape [1,1,12]
            encoded_alt_y = self.tcn_encoder_altitude_Y(altitude_y1) 
            
            #encoded_alt_y_repeated.shape [1, 12, 12]
