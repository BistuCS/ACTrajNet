...
from model.lstm_model import BiLstmNet2, BiLstmNet
...
class ACTrajNet(nn.Module):
    def __init__(self, args):
        ...
        self.adaptive_pool  = nn.AdaptiveAvgPool1d(output_size=11)  
        self.bilstm_encoder_altitude_X = BiLstmNet(input_size=lstm_input_size, output_size=n_classes_lstm, hidden_size=lstm_hidden_size, num_layers=lstm_layers, dropout=dropout)
        self.bilstm_encoder_altitude_Y = BiLstmNet(input_size=lstm_input_size, output_size=n_classes_lstm, hidden_size=lstm_hidden_size, num_layers=lstm_layers, dropout=dropout)
    ...
    def forward(self, x, y, adj,context,sort=False):        
        ...
        for agent in range(x.shape[2]):
            ...
            # encoded_x2.shape [1, 12, 11]
            encoded_x2 = self.tcn_encoder_x(x1) 


            # altitude_x1.shape [1,11,1]
            altitude_x1 = x[:, 2, agent][None, :, None]
            #encoded_alt_x.shape [1,12,1]
            encoded_alt_x = self.bilstm_encoder_altitude_X(altitude_x1)

           
            encoded_alt_x = encoded_alt_x.permute(0,  2, 1)  # [1,1,12]
            encoded_alt_x = self.adaptive_pool(encoded_alt_x)   # [1,1,11]
            encoded_alt_x_repeated = encoded_alt_x.repeat(1, 12, 1)
            t = torch.cat((encoded_x2, encoded_alt_x_repeated), dim=2)
            tt = self.relu_X(self.proj_X(t))
            #encoded_x2.shape  [1, 12, 11]
            encoded_x2 = self.lnorm_X(tt)
            encoded_x.shape [1, 1, 132]
            encoded_x = torch.flatten(encoded_x2)[None,None,:]

            c1 = torch.transpose(context[:,:, agent][None, :, :], 1, 2)
            encoded_context = self.context_conv(c1)
            # encoded_context shape torch.Size([1, 1, 10])
            encoded_context = self.relu(self.context_linear(encoded_context))
            
            # encoded_context shape torch.Size([1, 1, 7])
            #encoded_x.shape [1, 1, 132]  encoded_context.shape [1, 1, 7]  appended_x.shape [1, 1, 139]
            appended_x = torch.cat((encoded_x, encoded_context),dim=2) 
            
            encoded_appended_trajectories_x.append(appended_x)
            
            # y shape: torch.Size([12, 3, 1])
            y1 = torch.transpose(y[:,:, agent][None, :, :], 1, 2)
            #encoded_y2.shape [1, 12, 12]
            encoded_y2 = self.tcn_encoder_y(y1)
            #altitude_y1.shape  [1,12,1]
            altitude_y1 = y[:, 2, agent][None, :, None] 
            encoded_alt_y = self.bilstm_encoder_altitude_Y(altitude_y1) #encoded_alt_y.shape [1,12,1]
            encoded_alt_y = encoded_alt_y.permute(0,  2, 1) 
            encoded_alt_y_repeated = encoded_alt_y.repeat(1, 12, 1)        #1,12,12
            #encoded_y.shape [1, 12, 12]
            encoded_y = self.lnorm_Y(self.relu_Y(self.proj_Y(torch.cat((encoded_y2, encoded_alt_y_repeated), dim=2))))
            encoded_y = torch.flatten(encoded_y)[None,None,:]  # [1, 1, 144]
        ...
        for agent in range(x.shape[2]):
            ...
            # print('H_y shape', H_y.shape) torch.Size([1, 1, 144])
            # exit()
    ...
    def inference(self,x,z,adj,context):
        ...
        # pass all agents through encoder
        for agent in range(x.shape[2]):
            x1 = torch.transpose(x[:,:, agent][None, :, :], 1, 2)   #shape 1,3,11
            # x2 = x[:,:, agent][None, :, :]
            c1 = torch.transpose(context[:,:, agent][None, :, :], 1, 2)
            encoded_context = self.context_conv(c1)
            encoded_context = self.relu(self.context_linear(encoded_context))
            #encoded_x2.shape [1, 12, 11]
            encoded_x2 = self.tcn_encoder_x(x1)
            altitude_x = x[:, 2, agent][None, :, None]  #altitude_x.shape [1, 11, 1]
            #encoded_alt_x.shape [1, 12, 1]
            encoded_alt_x = self.bilstm_encoder_altitude_X(altitude_x)
            #encoded_alt_x.shape [1, 1, 12]
            encoded_alt_x = encoded_alt_x.permute(0, 2, 1)
            encoded_alt_x = self.adaptive_pool(encoded_alt_x)   # [1,1,11] 
            #encoded_alt_x_repeated.shape [1, 12, 11]
            encoded_alt_x_repeated = encoded_alt_x.repeat(1, 12, 1)  

            encoded_x = self.lnorm_X(self.relu_X(self.proj_X(torch.cat((encoded_x2, encoded_alt_x_repeated), dim=2))))
