...
from model.lstm_model import BiLstmNet
...
class ACTrajNet(nn.Module):
    ...
    def forward(self, x, y, adj,context,sort=False):        
        ...
        for agent in range(x.shape[2]):
            ...
            encoded_x1 = self.tcn_encoder_x(x1) # [1, 12, 11]
            ...
            #[1,12,1]
            encoded_alt_y = self.bilstm_encoder_altitude_Y(altitude_y1)
            #[1,1,12]
            encoded_alt_y = encoded_alt_y.permute(0,  2, 1)


            # y1 shape: torch.Size([1, 3, 12])
            encoded_y1 = self.tcn_encoder_y(y1)
            encoded_y1 = encoded_y1 * torch.transpose(self.sig_a_Y(self.fc_a_Y(encoded_alt_y)), 1, 2)
   
            encoded_y = torch.flatten(encoded_y1)[None,None,:]
    ...
    def inference(self,x,z,adj,context):
        ...
        for agent in range(x.shape[2]):
            ...
            encoded_alt_x = self.adaptive_pool(encoded_alt_x)
